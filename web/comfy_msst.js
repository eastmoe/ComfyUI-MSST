...
function findModelEntry(data, selected) {
    ...
    const normalized = String(selected).replaceAll("\\", "/");
    ...
    for (const [key, value] of Object.entries(models)) {
        if (key === normalized || key.endsWith(`/${normalized}`) || key.endsWith(`/${filename}`)) {
            return value;
        }
    }
    return null;
...
function refreshNodeInfo(node, infoWidget) {
    ...
    const selected = modelWidget?.value;
    loadModelInfo().then((data) => {
        infoWidget.lines = formatEntry(data.models?.[selected]);
...
function wrapModelWidgetCallback(node, afterChange) {
    ...
    if (!modelWidget || modelWidget.comfyMsstWrapped) {
        return;
    }
